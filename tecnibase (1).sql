...
-- Índices 
create index idxCedula_Cliente
on cliente(RUC_empresa);

create index idxNumero_Factura
on factura(nro_factura);

create index idxFecha_servicio
on servicio_prestado(fecha);

create index idxCedula_empleado
on empleado(cedula);

create index idxProveedor_Producto
on producto((Lower(proveedor)));
...
-- Visualizar y agregar una factura
grant select, insert
on tecnibase.factura
to 'Cajero'@'localhost';
flush privileges;
-- Visualizar y agregar cliente
grant select, insert
on tecnibase.cliente
to 'Cajero'@'localhost';

-- Visualizar y agregar servicio prestado
grant select, insert
on tecnibase.servicio_prestado
to 'Cajero'@'localhost';

-- Visualizar productos del inventario
grant select
on tecnibase.producto
to 'Cajero'@'localhost';

-- Permiso para la vista detalleVentas
grant select
on tecnibase.detalleVentas
to 'Cajero'@'localhost';


-- Permiso para el SP sp_insert_cliente
grant execute 
on procedure tecnibase.sp_insert_cliente
to 'Cajero'@'localhost';

-- Permisos de tecnico
create user 'Tecnico'@'localhost' identified by 'tecnico123';

-- Visualizar, agregar o actualizar un servicio tecnico
grant select, insert, update
on tecnibase.servicio_tecnico
to 'Tecnico'@'localhost';
...
-- Permiso para la vista infoServicioTecnico
grant select
on tecnibase.infoServicioTecnico
to 'tecnico'@'localhost';


create user 'Inventario_app'@'localhost' identified by 'inventario123';

-- Visualizar y actualizar productos
grant select, update
on tecnibase.producto
to 'Inventario_app'@'localhost';

-- Visualizar y actualizar catalogo
grant select, update
on tecnibase.catalogo
to 'Inventario_app'@'localhost';

create user 'Reportes_app'@'localhost' identified by 'reportes123';

-- Visualizar facturas
grant select
on tecnibase.factura
to 'Reportes_app'@'localhost';
...
SELECT 
    ...
    S.tipo_servicio
FROM Factura F
JOIN Servicio_prestado S ON F.id_servicio = S.id_servicio
JOIN Cliente C ON S.id_cliente = C.id_cliente
LEFT JOIN Venta V ON S.id_servicio = V.id_servicio
LEFT JOIN Venta_producto VP ON V.id_servicio = VP.id_servicio AND V.id_venta = VP.id_venta
LEFT JOIN Producto P ON VP.id_producto = P.id_producto;


CREATE VIEW infoServicioTecnico AS
SELECT 
    E.id_empleado,
    E.nombre AS nombre_empleado,
    EQ.id_equipo,
    EQ.modelo AS modelo_equipo,
    EQ.tipo_equipo,
    EQ.estado_equipo,
    ST.descripcion AS descripcion_servicio,
    C.nombre AS nombre_cliente,
    C.apellido_persona AS apellido_cliente
FROM Servicio_tecnico ST
JOIN Empleado E ON ST.idEmpleado = E.id_empleado
JOIN Equipo EQ ON ST.id_equipo = EQ.id_equipo
JOIN Cliente C ON EQ.id_cliente = C.id_cliente;

CREATE VIEW infoInventario AS
SELECT 
    P.id_producto,
    P.marca,
    P.descripcion,
...
FROM Producto P
JOIN Catalogo CAT ON P.id_catalogo = CAT.id_catalogo;

CREATE VIEW facturaServicio AS
SELECT 
    F.nro_factura,
    F.fecha_emision,
    F.monto_total,
    S.tipo_servicio,
    CASE 
        WHEN S.tipo_servicio = 'S' THEN 'Servicio Técnico'
        WHEN S.tipo_servicio = 'V' THEN 'Venta Directa'
        ELSE 'Otro'
